...
<body>
<form  id="form1" action="provider/search.action" method="post">
  ...
  <tr>
	...
	<tr>
	  <td>关键字：</td>
	   ...
	   <td>状态：</td>
		<td>
			<select name="providerDto.status" >
				<option value="-1" >全部</option>
				<option value="0" <c:if test="${providerDto.status==0}">selected="selected"</c:if>>有效</option>
				<option value="1" <c:if test="${providerDto.status==1}">selected="selected"</c:if>>无效</option>
			</select>
	   </td>
...
<c:forEach items="${request.result.queryList}" var="providerInfo" >
	<tr>
	    ...
	    <td><fmt:formatDate  value="${providerInfo.validdate}" pattern="yyyy-MM-dd" /></td>
	    <td>
	    	<a class="bts_link" href="provider/show.action?id=${providerInfo.proid}" target="_blank">详情</a> 
	    	<a class="bts_link" href="javascript:delTips('','provider/delete.action?proId=${providerInfo.proid}');" >删除</a> 
	    	<c:if test="${providerInfo.validstatus==0}">
				<a class="bts_link" href="javascript:delTips('您确定要屏蔽这条信息？','provider/invalid.action?proId=${providerInfo.proid}');" >屏蔽</a> 
			</c:if>
	    	<c:if test="${providerInfo.validstatus==1}">
	    		<a class="bts_link" href="javascript:delTips('您确定要恢复这条信息？','provider/enable.action?proId=${providerInfo.proid}');" >恢复</a> 
	    	</c:if>
	    </td>
	</tr>
