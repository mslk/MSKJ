...
<body>
<form  id="form1" action="transportSupply/search.action" method="post">
<table class="publictable" width="90%" border="0" cellspacing="1" cellpadding="0">
  <tr>
	...
	<td >关键字：</td>
		...
		<td>状态：</td>
		<td>
			<select name="transportSupplyDto.status" >
				<option value="-1" >全部</option>
				<option value="0" <c:if test="${transportSupplyDto.status==0}">selected="selected"</c:if>>有效</option>
				<option value="1" <c:if test="${transportSupplyDto.status==1}">selected="selected"</c:if>>无效</option>
			</select>&nbsp;&nbsp;&nbsp;&nbsp;
...
<c:forEach items="${request.result.queryList}" var="transportSupply" >
	<tr>
	    ...
	    <td>
	    	<a class="bts_link" href="transportSupply/show.action?id=${transportSupply.id}" target="_blank">详情</a>
	    	<a class="bts_link" href="javascript:delTips('','transportSupply/delete.action?supplyId=${transportSupply.id}');" >删除</a>  
	    	<c:if test="${transportSupply.validstatus==0}">
				<a class="bts_link" href="javascript:delTips('您确定要屏蔽这条信息？','transportSupply/invalid.action?supplyId=${transportSupply.id}');" >屏蔽</a> 
			</c:if>
	    	<c:if test="${transportSupply.validstatus==1}">
	    		<a class="bts_link" href="javascript:delTips('您确定要恢复这条信息？','transportSupply/enable.action?supplyId=${transportSupply.id}');" >恢复</a> 
	    	</c:if>
	    </td>
	</tr>
